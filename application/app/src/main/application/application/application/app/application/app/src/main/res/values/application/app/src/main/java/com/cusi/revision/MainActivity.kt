...
fun AccountScreen(
    ...
    Column(Modifier.fillMaxSize().padding(20.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
        ...
        if(!loggedIn){
            ...
            Button(
                enabled=registerName.isNotBlank()&&email.contains("@")&&password.length>=6,
                onClick={
                    prefs.edit().putBoolean("logged_in",true).putString("email",email)
                        .putString("student_name",registerName).apply()
                    onLoginChange(true)
                    message="Compte local créé. Le branchement serveur est prêt mais nécessite l'URL du backend."
                },
            ...
            OutlinedButton(
                ...
                onClick={
                    prefs.edit().putBoolean("logged_in",true).putString("email",email).apply()
                    onLoginChange(true)
            ...
            Text("Connecté : ${prefs.getString("email","")}")
            Text("Synchronisation en ligne : prête à être branchée au serveur.")
            OutlinedButton(onClick={
                prefs.edit().putBoolean("logged_in",false).apply()
                onLoginChange(false)
            },Modifier.fillMaxWidth()){Text("Se déconnecter")}
        }
    }
}

@Composable
fun Certificate(name:String,prefs:android.content.SharedPreferences){
    val arr=JSONArray(prefs.getString("attempts","[]"))
    var best=0
    var bestTotal=0
    for(i in 0 until arr.length()){val o=arr.getJSONObject(i);if(o.optBoolean("passed")&&o.optInt("percent")>=best){best=o.optInt("percent");bestTotal=o.optInt("total")}}
    Column(Modifier.fillMaxSize().padding(20.dp),verticalArrangement=Arrangement.spacedBy(14.dp)){
        Text("🏆 Attestation de réussite",style=MaterialTheme.typography.headlineSmall)
        if(name.isBlank())Text("Renseigne d'abord ton nom dans Profil.")
        else if(best==0)Text("Aucun examen réussi à 50 % ou plus. Réussis un examen blanc pour débloquer l'attestation.")
        else{
            Card(Modifier.fillMaxWidth()){Column(Modifier.padding(22.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
                Text("ATTESTATION DE RÉUSSITE",style=MaterialTheme.typography.titleLarge)
                Text("Certificat Unique en Soins Infirmiers (CUSI)")
                Text("Décernée à")
                Text(name,style=MaterialTheme.typography.headlineSmall)
                Text("Résultat : $best % ($bestTotal questions)")
                Text("Attestation générée dans l'application après réussite.")
            }}
        }
    }
}
